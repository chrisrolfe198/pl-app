    ...
    <div>
      ...
      {Object.entries(childObjects).map(item => {
        if (utils.isFunction(item[1])) {
          ...
          return (<ListItemElement objectIndex={props.objectIndex.slice()} data={props.data} fullscreen={props.fullscreen} key={item[0]} listItems={childObjects} />)
        } else {
          return (<ListElement objectIndex={props.objectIndex.slice()} data={props.data} fullscreen={props.fullscreen} key={item[0]} title={item[0]} listItems={item[1]} />)
        }
      })}
    </div>
  );
